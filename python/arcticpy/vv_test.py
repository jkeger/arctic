...
class VVTestBench:
    ...
    def __init__(
        ...
        sigma_clipping_threshold=3.0,  # used while calculating which columns contain stars
        sigma_clipping_niter=10.0,  # used while calculating which columns contain stars
    ):
    ...
    def print(self, *args, **kwargs):
        if self.verbose:
            print(*args, **kwargs)  # don't print if not verbose
    ...
    def test(
        ...
        if (
            self.parallel_ccd is not None
            and self.parallel_roe is not None
            and self.parallel_traps is not None
        ):
        ...
        if (
            self.serial_ccd is not None
            and self.serial_roe is not None
            and self.serial_traps is not None
        ):
    ...
    def _test1d(
        ...
        """
        if y_ccd is None:
            raise Exception("CCD well filling model required to interpret V&V!")
        if y_roe is None:
            raise Exception("ROE model required to define overscan region!")
        if y_roe.overscan_start < 0:
            raise Exception("No overscan region is defined!")
        if y_traps is None:
            raise Exception("CTI model required to define test trail profile!")
        if model_bias is None:
            model_bias = 0.0
        if fit_bias is None:
            fit_bias = True
        if iterate_bias is None:
            iterate_bias = False
        ...
        if x_roe is not None:
            ...
            if x_max < x_min:
                raise Exception("No pixel areas recognised")
        ...
        if valid_columns is None:
            ...
            for iteration in np.flip(np.arange(self.sigma_clipping_niter)):
                ...
                background_rms = np.sqrt(
                    np.mean((background_image[valid_pixels] - background_level) ** 2)
                )
        ...
        if pixels_pre_cti is None:
            ...
            pixels_pre_cti = (image[0 : y_roe.overscan_start, x_min:x_max])[
                :, valid_columns
            ]
        ...
        if model_trap_density <= 0:
            raise Exception("No traps in CTI model!")
        ...
        if self.sum_of_exponentials:
            ...
            traps_effect_on_final_pixel = y_roe.overscan_start * (
                ...
                - y_ccd.phases[0].volume(model_bias)
            )
            model_overscan = np.full(len(overscan), model_bias)
            for trap in y_traps:
                model_overscan += (
                    trap.density
                    * traps_effect_on_final_pixel
            ...
            while np.sum(
                model_pre_cti[
                    y_roe.overscan_start - nrows_buffer : y_roe.overscan_start
                ]
            ) < (
                n_traps_seen_by_final_pixel
                * 3  # Increase to engineering standard 3x safety margin
            ):
                nrows_buffer += 1
        ...
        if fit_bias:
            ...
            M = np.vstack(
                (model_overscan - model_bias, np.zeros_like(model_overscan))
            ).T
            best_fit_coefficients, residuals, rank, singular_values = np.linalg.lstsq(
                M * weight[:, np.newaxis], (overscan - model_bias) * weight, rcond=None
            )
            best_fit_coefficients[1] = model_bias
        best_fit_overscan = best_fit_coefficients[1] + best_fit_coefficients[0] * (
            model_overscan - model_bias
        )
        ...
        best_fit_bias = best_fit_coefficients[
            1
        ]  # + best_fit_coefficients[1] * model_bias
        model_volume_filled = y_ccd.phases[0].volume(
            model_pre_cti[y_roe.overscan_start - 1]
        )
        model_volume_active = model_volume_filled - y_ccd.phases[0].volume(model_bias)
        best_fit_volume_active = model_volume_filled - y_ccd.phases[0].volume(
            best_fit_bias
        )
        ...
        self.print(
            "Inferred best-fit model (bias: ",
            best_fit_bias,
            ", trap density: ",
            best_fit_trap_density,
            ")",
        )
