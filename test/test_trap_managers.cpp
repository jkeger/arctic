...
TEST_CASE("Test initialisation", "[trap_managers]") {
    ...
    SECTION("Initial watermarks") {
        ...
        REQUIRE(trap_manager.watermark_fills.size() == 7);
        REQUIRE(trap_manager.watermark_fills[0].size() == 2);
        REQUIRE(
            trap_manager.watermark_volumes.sum() ==
            trap_manager.n_watermarks * trap_manager.empty_watermark);
        REQUIRE(
            trap_manager.watermark_fills[0].sum() ==
            trap_manager.n_traps * trap_manager.empty_watermark);
        ...
        REQUIRE(
            trap_manager_2.watermark_fills[0].sum() ==
            trap_manager_2.n_traps * trap_manager_2.empty_watermark);
        ...
        REQUIRE(
            trap_manager_ic.watermark_fills[0].sum() ==
            trap_manager_ic.n_traps * trap_manager_ic.empty_watermark);
    }
}
...
//         TrapManagerInstantCapture trap_manager(
//             std::valarray<Trap>{trap_1, trap_2}, 6, ccd_2);
//         trap_manager.initialise_trap_states();
//         trap_manager.set_fill_probabilities_from_dwell_time(1.0);
//         trap_manager.n_active_watermarks = 4;
//         trap_manager.i_first_active_wmk = 2;
