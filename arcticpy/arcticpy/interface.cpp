...
void add_cti(
    ...
    std::valarray<Trap> traps_instant_capture(TrapInstantCapture(0.0, 0.0), n_traps_instant_capture);
    ...
    for (int i_trap = n_traps_standard; i_trap < n_traps_instant_capture; i_trap++) {
        ...
        traps_instant_capture[i_trap].release_timescale = trap_release_timescales[i_trap];
    }
